...
namespace olc
{
	...
	olc::rcode PixelGameEngine::Construct(int32_t screen_w, int32_t screen_h, int32_t pixel_w, int32_t pixel_h, uint8_t flags)
	{
		bPixelCohesion = flags & olc::FLAG::COHESION;
		vScreenSize = { screen_w, screen_h };
		vInvScreenSize = { 1.0f / float(screen_w), 1.0f / float(screen_h) };
		vPixelSize = { pixel_w, pixel_h };
		vWindowSize = vScreenSize * vPixelSize;
		bFullScreen = flags & olc::FLAG::FULLSCREEN;
		bEnableVSYNC = flags & olc::FLAG::VSYNC;
	...
	HWButton PixelGameEngine::GetKey(olc::Key k) const
	{
	...
	void PixelGameEngine::DrawSprite(const olc::vi2d& pos, Sprite* sprite, uint32_t scale, olc::Sprite::Flip flip)
	{
		DrawSprite(pos.x, pos.y, sprite, scale, flip);
	}

	void PixelGameEngine::DrawSprite(int32_t x, int32_t y, Sprite* sprite, uint32_t scale, olc::Sprite::Flip flip)
	{
		if (sprite == nullptr)
			return;
	...
	void PixelGameEngine::DrawPartialSprite(const olc::vi2d& pos, Sprite* sprite, const olc::vi2d& sourcepos, const olc::vi2d& size, uint32_t scale, olc::Sprite::Flip flip)
	{
		DrawPartialSprite(pos.x, pos.y, sprite, sourcepos.x, sourcepos.y, size.x, size.y, scale, flip);
	}

	void PixelGameEngine::DrawPartialSprite(int32_t x, int32_t y, Sprite* sprite, int32_t ox, int32_t oy, int32_t w, int32_t h, uint32_t scale, olc::Sprite::Flip flip)
	{
		if (sprite == nullptr)
			return;
